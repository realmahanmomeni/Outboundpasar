...
const Settings = () => {
  ...
  const isDetailPage = /^\/nodes\/\d+/.test(location.pathname)
  const isNativePage = location.pathname === '/nodes/native'

  const handleCreateNode = useCallback(() => {
    if (!canCreateNodes) return
    window.dispatchEvent(new CustomEvent('openAddPanelDialog'))
  ...
  useEffect(() => {
    if (isCoreEditorPage || isDetailPage || isNativePage || visibleTabs.length === 0) return
    const currentTab = visibleTabs.find(tab => location.pathname === tab.url)
    if (!currentTab) {
      navigate(visibleTabs[0].url, { replace: true })
    }
  }, [isCoreEditorPage, isDetailPage, isNativePage, location.pathname, navigate, visibleTabs])
